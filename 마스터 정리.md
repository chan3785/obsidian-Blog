...
1. **첫 번째 경우**: $(f(n))$이 $(n^{\log_{b{a}}} = h(n))$보다 엄격하게 작을 때 = $h(n)$이 더 무거울 때
    $((f(n) = O(n^{\log_b{a}-\epsilon}))$ for some $(\epsilon > 0)), (T(n) = \Theta(n^{\log_b{a}}))$

1. **두 번째 경우**: $(f(n))$이 $(n^{\log_{b{a}}} = h(n))$와 같을 때 $((f(n) = \Theta(n^{\log_b{a}}\log^k{n}))$ for some $(k \geq 0)), (T(n) = \Theta(n^{\log_b{a}}\log^{k+1}{n}))$
