...
#### 상수 정의하기
고 언어에서 상수를 선언할 때, 데이터 타입을 명시적으로 지정하지 않아도 된다. 이는 고 언어의 상수는 "타입이 없는 상수(untyped constant)"로 간주될 수 있기 때문입니다. 이러한 상수는 사용되는 문맥에 따라 적절한 타입으로 자동으로 변환됩니다. 그러나 필요에 따라 명시적으로 타입을 지정할 수도 있습니다
### 출처(참고문헌)
-
